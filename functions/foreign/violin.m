...
medc='r';
b=[]; %bandwidth
plotlegend=0; % I changed it from 1 to 0, TT (09.05.2022.)
plotmean=1;
plotmedian=1;
x = [];
%_____________________
...
if isempty(x)
    ...
    setX = x; % i changed from 1 to x, TT 10.05.2022.
    if isempty(xL)==0
...
for i=i:size(Y,2)
    ...
    else%if setX == 1 % I commented elseif out, TT 10.05.2022.
        if plotmean == 1
...
if setX == 0
    ...
    %axis([min(x)-0.05*range(x) max(x)+0.05*range(x), min(U(:))max(U(:))]); % I commented this out, TT 10.05.2022.
end
